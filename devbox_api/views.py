...
def api_usage_post(request):
    serializer = APIUsageSerializer(data=request.data)
    if serializer.is_valid():
        # Simulate Kafka log
        print(f"[Kafka Event] New API usage entry: {serializer.validated_data}")
...
def api_usage_get(request):
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            # Simulate Kafka log
            print(f"[Kafka Event] New API usage: {serializer.validated_data}")
            MOCK_API_USAGE.append(serializer.validated_data)
            return Response({"message": "API usage logged successfully."}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
